...
class musicCommands(commands.Cog):
    ...
    async def _join(self, ctx: commands.Context):
        if not self.queue is None:
            return await ctx.send("Bot is already connected to a channel")
    ...
    async def _leave(self, ctx: commands.Context):
        if self.queue is None:
            return await ctx.send("The bot is not connected to any channel")
        if ctx.author.voice and ctx.author.voice.channel is not self.queue.vc.channel:
            return await ctx.send("You must be on the same channel as the bot")
    ...
    async def _stream(self, ctx: commands.Context, *, search: str):
        ...
        ytdlSource = YTDLSource(search, ctx.author)
    ...
    async def _pause(self, ctx: commands.Context):
        author = ctx.author
        if self.queue is None:
            return await ctx.send("Bot is not connected to any channel")
        if author.voice and author.voice.channel is not self.queue.vc.channel:
            return await ctx.send("You must be on the same channel as the bot!")
        if not self.queue.vc.is_playing():
    ...
    async def _resume(self, ctx: commands.Context):
        ...
        if self.queue is None:
            return await ctx.send("Bot is not connected to any channel")
        if self.queue.vc.channel is not author.voice.channel:
            return await ctx.send("You're not on the same channel as the bot!")
    ...
    async def _now_playing(self, ctx: commands.Context):
        if self.queue is None:
            return await ctx.send("Bot is connected to any channel")
        if self.queue.vc and not self.queue.vc.is_playing():
            return await ctx.send("Bot is not playing anything!")
    ...
    async def _loop(self, ctx: commands.Context):
        if self.queue is None:
            return await ctx.send("Bot is not connected to any channel")
        if self.queue.vc and self.queue.vc.channel is not ctx.author.voice.channel:
            return await ctx.send("You must be in the same channel as the bot")
        if not self.queue.vc.is_playing():
            return await ctx.send("Bot is not playing anything!")
    ...
    @commands.command(aliases=['clearqueue', 'clearq'])
    ...
    async def _delete(self, ctx: commands.Context, index):
        if self.queue is None:
            return await ctx.send("Bot is not playing anything yet!")
        try:
            index = int(index)
        ...
        if 1 < index <= len(self.queue.queue):
            await ctx.send(f"Song ``{self.queue.queue[index - 1]}`` has been deleted")
    ...
    async def _shuffle(self, ctx: commands.Context):
        if self.queue is None or self.queue.vc and not self.queue.vc.is_playing():
            return await ctx.send("Bot is not playing anything yet!")
